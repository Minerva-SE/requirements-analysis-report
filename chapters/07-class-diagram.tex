...
\hline
Administrator & id, name, permission, auditData(), manageConfig() & 用户审批、数据审核与系统管理 \\
\hline
...
\item 业务对象类：Experiment（实验）、EEGData（EEG 数据）、QualityReport（质控报告）、DataAccessRequest（数据使用申请与授权）、ConsentRecord（知情同意记录）、AnalysisTask（分析任务）以及 OperationLog（操作日志）。
\end{itemize}
